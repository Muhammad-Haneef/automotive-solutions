<x-layouts.admin>
                                    ...
                                    @forelse ($rows as $row)
                                        ...
                                        <div class="card card{{ $row->id }}">
                                            <div class="card-header">
                                                <h5>
                                                    <button class="btn btn-link collapsed ps-3"
                                                        data-bs-toggle="collapse"
                                                        data-bs-target="#collapseicon{{ $row->id }}"
                                                        aria-expanded="false" aria-controls="collapseicon2">
                                                        {{ $row->title }}
                                                    </button>
                                                </h5>
                                            </div>
                                            <div class="collapse" id="collapseicon{{ $row->id }}"
                                                    ...
                                                    <div class="py-2 text-center border border-info mb-3">
                                                        <div class="row">
                                                            <div class="col-md-2">
                                                                @if ($row->show_title)
                                                                    <i
                                                                        class="icofont icofont-check-circled text-success"></i>
                                                                @else
                                                                    <i
                                                                        class="icofont icofont-close-circled text-danger"></i>
                                                                @endif
                                                                Show Section Title
                                                            </div>
                                                            <div class="col-md-2">
                                                            ...
                                                            <div class="col-md-1">
                                                                <div class="fw-normal text-sm fw-light text-end"
                                                                    ...
                                                                    <div class="common-align gap-2 justify-content">
                                                                        ...
                                                                        <i onclick="showAddVideoModal('<?= route('admin.store-product-video', [$pid]) ?>', <?= $pid ?>, <?= $row->id ?>)"
                                                                            class="icofont icofont-video-alt cursor-pointer  fs-5">
                                                                        </i>

                                                                        <a class="square-white"
                                                                            href="{{ route('admin.edit-' . $rsn, [$pid, $row->id]) }}"
                                                    ...
                                                    <div class="row">
                                                        <div class="col-12">
                                                            <div class="table-responsive custom-scrollbar">
                                                                <table
                                                                    class="display table-striped border data-table_ table-sm"
                                                                    style="width: 100%;">
                                                                    <thead>
                                                                            ...
                                                                            <tr>
                                                                                <td class=" text-center">
                                                                                    <a target="_blank"
                                                                                        href="{{ $v->url }}">
                                                                                        <i
                                                                                            class="icofont icofont-video-alt cursor-pointer text-danger"></i>
                                                                                    </a>
                                                                                </td>
                                                                                <td>
                                                                                    {{ $v->title }}
                                                                                ...
                                                                                <td class=" text-center">
                                                                                    ...
                                                                                    <span id="videoRow<?= $v->id ?>"
                                                                                        onclick="editVideo('<?= route('admin.update-product-video', [$pid, $v->id]) ?>', <?= $pid ?>, <?= $row->id ?>, <?= $v->id ?>)"
                                                                                        data-video_url="{{ $v->url }}"
                                                                                        data-video_title="{{ $v->title }}"
                                                                                        data-video_sort_by="{{ $v->sort_by }}">
                                                                                        <svg width="18"
                                                                                            height="18">
                                                                                            <use
                                                                                                href="http://127.0.0.1:8000/assets/svg/icon-sprite.svg#edit-content">
    ...
    <div class="modal fade" id="add_video_modal" tabindex="-1" role="dialog" aria-labelledby="add_video_modal"
        aria-hidden="true">
                    ...
                    <form method="post" id="modal_form" action=''>
                        ...
                        <div class="modal-body">
                            <div class="row g-3">
                                ...
                                <input name="product_video_section_id" id="midal_product_video_section_id"
                                    type="hidden" value="" />
                                <x-admin.form.input ref="title" title="Video Caption/Title" required=1 />
                                <x-admin.form.textarea ref="url" rows="2"
                                    title="Youtybe Video Embeded Code" required=1 />
                                <x-admin.form.input type="number" ref="sort_by" title="Sort By"
                                    value="{{ $row ? $row->sort_by : old('sort_by') }}" class="col-md-6" />
                            </div>
                        </div>
