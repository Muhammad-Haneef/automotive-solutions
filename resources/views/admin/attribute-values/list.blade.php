<x-admin.data-listing-card>
    ...
    <x-slot:before>
        <form method="post"
            action="{{ $row ? route('admin.update-' . $rsn, [$row->id]) : route('admin.store-' . $rsn) }}">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="row g-3">
                        <x-admin.form.dropdown ref="attribute_id" title="Attribute" :options="$attributes"
                            selected="{{ $row ? $row->attribute_id : old('attribute_id') }}" class="col-md-3" />
                        <x-admin.form.input ref="title" title="Title" value="{{ $row ? $row->title : old('title') }}"
                            required=1 class="col-md-5" />
                        ...
                        <x-admin.form.submit-btn title="{{ $row ? 'Update' : 'Submit' }}"
                            class="col-md-12 text-end mt-0" />
                    </div>
                </div>
            </div>
        </form>
        ...
        @forelse ($rows as $index => $row)
            <tr class="{{ $row->deleted_at ? 'deleted' : '' }}">
                ...
                <x-admin.action-btn route="{{ $rsn }}" id="{{ $row->id }}" privilegeEditId="0"
                    privilegeDeleteId="0" privilegeRestoreId="{{ $row->deleted_at ? 1 : 0 }}" />
            </tr>
        @empty
        @endforelse
    </x-slot:body>
