    ...
    <x-slot:before>
        <form method="post" action="{{ $row ? route('admin.update-' . $rsn, [$row->id]) : route('admin.store-' . $rsn) }}">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row g-3">
                        <x-admin.form.dropdown ref="input_type" title="Input Type" :options="getAttributeInputTypes()" selected="{{$row ? $row->input_type : old('input_type')}}" class="col-md-3" />
                        ...
                        <x-admin.form.submit-btn title="{{ $row ? 'Update' : 'Submit' }}" class="col-md-2" />
                    </div>
                </div>
            </div>
        </form>
