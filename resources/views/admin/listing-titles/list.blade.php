    ...
    <x-slot:before>
        <form method="post" action="{{ $row ? route('admin.update-' . $rsn, [$row->id]) : route('admin.store-' . $rsn) }}">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row g-3">
                        <x-admin.form.input ref="title" title="Title" value="{{$row ? $row->title : old('title')}}" required=1 class="col-md-6" />
                        <x-admin.form.input type="number" ref="sort_by" title="Sort By" value="{{$row ? $row->sort_by : old('sort_by', 1)}}" class="col-md-3" />
                        ...
                        <x-admin.form.input ref="description" title="Description" value="{{$row ? $row->description : old('description')}}" class="col-md-10" />
