jQuery(document).ready(function($) {
	...
	$(document).on('click', '.site-module-tab li:not(active) a', function(event){
        ...
        var data = {
			...
			'mobile': $find.data('items-mobile'),
			'tablet': $find.data('items-tablet'),
			...
			'product_brands': $find.data('product_brands'),
		...
		$.post(MyAjax.ajaxurl, data, function(response) {
		    ...
		    $thisbutton.closest('.site-module-tab').find('.header-tab-link').removeClass('active');
			...
			ignavoThemeModule.productquantity();
			
			ignavoThemeModule.variationform();	
