...
class BlogCategoryController extends Controller
{
    ...
    {
        $this->data['rows'] = BlogCategory::latest()->withTrashed()->get();
    ...
    {
        BlogCategory::create($request->only((new BlogCategory())->getFillable()));
        return back()->with([
    ...
    {
        if (!$this->data['row'] = BlogCategory::find($id)) {
    ...
    {
        BlogCategory::where('id', $id)->update($request->only((new BlogCategory())->getFillable()));
        return redirect()->route('admin.' . $this->data['rpn'])->with([
            'message' => 'Saved successfully.',
            'alert-type' => 'success'
        ]);
    ...
    public function destroy($id)
    {
        DB::beginTransaction();
        try {
            BlogCategory::withTrashed()->findOrFail($id)->forceDelete();
            DB::commit();
            return back()->with([
