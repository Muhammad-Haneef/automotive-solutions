...
class CategoryController extends Controller
{
    ...
    public function store(StoreCategoryRequest $request)
    {
        ...
        if ($request->hasFile('icon')) {
            $data['icon'] = $request->file('icon')->store($this->data['rpn'], 'public');
        } else {
    ...
    public function update(UpdateCategoryRequest $request, Category $category, $id)
    {
        ...
        if ($request->hasFile('icon')) {
            $data['icon'] = $request->file('icon')->store($this->data['rpn'], 'public');
            deleteImage($request->old_icon);
        } else {
            $data['icon'] = $request->old_icon;
        }
        // Save only fillable fields in the database
        Category::where('id', $id)->update(array_intersect_key($data, array_flip((new Category())->getFillable())));
